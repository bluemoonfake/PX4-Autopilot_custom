...
Example:
    python3 Tools/antenna_tracker/usb_router.py --device /dev/ttyACM0

Then run QGC without opening /dev/ttyACM0 directly, and send a simulated target with:
    python3 Tools/antenna_tracker/send_target.py --port udpout:127.0.0.1:18570 --sysid 2

This is a bench-routing utility, not a firmware safety boundary. Keep a
separate servo-power cutoff and record the router port mapping in evidence.
"""
